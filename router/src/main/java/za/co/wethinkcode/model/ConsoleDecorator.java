...
public class ConsoleDecorator {
    ...
    public String viewMessage(String message, String type) throws NullPointerException {
        ...
        if (type.toLowerCase().contains("error") || type.toLowerCase().contains("normal"))
            mesType = (type.toLowerCase().contains("error") ? ("[" + red + "ERROR" + reset + "] ") : ("[" + orange+ "FIX-ME" + reset + "] "));
    ...
    public void displayFixMessage(String message) throws NullPointerException {
        ...
        if (message.contains("8=FIX.4.2")) {
            ...
            if (message.contains("39=")) {
                System.out.println(this.viewMessage("[" + orange + "BROKER" + reset + " : " + receiverValue + "] < [" + blue + "MARKET" + reset + " : " + senderValue + "] " + message, "none"));
                char fulfilled = message.charAt(message.indexOf("39=") + 3);

                if (fulfilled == '2')
                    System.out.println(this.viewMessage("[" + orange+ "BROKER" + reset + " : " + receiverValue + "] < [" + blue + "MARKET" + reset + " : " + senderValue + "] request has been " + green + "ACCEPTED" + reset, "none"));
                if (fulfilled == '8')
                    System.out.println(this.viewMessage("[" + orange+ "BROKER" + reset + " : " + receiverValue + "] < [" + blue + "MARKET" + reset + " : " + senderValue + "] request has been " + red + "REJECTED" + reset, "none"));
            } else {
                System.out.println(this.viewMessage("[" + orange+ "BROKER" + reset + " : " + senderValue + "] > [" + blue + "MARKET" + reset + " : " + receiverValue + "] " + message, "none"));
            }
        }
